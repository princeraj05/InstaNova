...
export default function Messages() {
  ...
  // ── Avatar — simple img tag, not a sub-component
  // (sub-components defined inside parent cause remount on every render → focus lost)
                  ...
                  {messages.map((m, i) => {
                    ...
                    return (
                      <div key={m._id || i} className={`flex items-end gap-2 ${isMine ? "flex-row-reverse" : "flex-row"}`}>
                        ...
                        <div className={`flex flex-col gap-1 max-w-[65vw] ${isMine ? "items-end" : "items-start"}`}>
                  ...
                  {messages.map((m, i) => {
                    ...
                    return (
                      <div key={m._id || i} className={`flex items-end gap-2 ${isMine ? "flex-row-reverse" : "flex-row"}`}>
                        ...
                        <div className={`flex flex-col gap-1 max-w-xs md:max-w-sm lg:max-w-md ${isMine ? "items-end" : "items-start"}`}>
                          <div className={`rounded-2xl overflow-hidden shadow-sm ${isMine ? "bg-indigo-500 text-white rounded-br-none" : "bg-white text-gray-800 rounded-bl-none border border-gray-100"}`}>
                            {m.text && <p className="px-4 py-2.5 text-sm leading-relaxed break-words">{m.text}</p>}
                            {m.reel && (
                              <div onClick={() => navigate(`/reels?reelId=${m.reel._id || m.reel}`)} className="cursor-pointer group">
