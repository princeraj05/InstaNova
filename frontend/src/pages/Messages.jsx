...
export default function Messages() {
  ...
  const [conversationUsers, setConversationUsers] = useState({}) // 🔥 stores friendId -> user info
  ...
  // LOAD OLD CONVERSATIONS + FETCH USER INFO FOR EACH
  useEffect(() => {
    const fetchConversations = async () => {
      try {
        ...
        // Fetch user info for each conversation friend
        const userMap = {}
        await Promise.all(
  ...
  const sendMessage = async () => {
    ...
    const newMsg = { conversationId: conversation._id, sender: userId, text: message }
  ...
  return (
    <div className="flex min-h-screen" style={{ background: "#f0f2f5" }}>
      ...
      <div className="w-full md:ml-64 flex h-screen overflow-hidden">
        ...
        >
          ...
          <div style={{ padding: "20px 16px 12px", borderBottom: "1px solid #f0f0f0" }}>
            <h2 style={{ fontSize: "20px", fontWeight: "700", color: "#0f0f0f", marginBottom: "12px" }}>
              Messages
            </h2>
                      ...
                      {!isMine && (
                        <img
                          ...
                          style={{
                            ...
                            objectFit: "cover", flexShrink: 0, marginBottom: "2px",
